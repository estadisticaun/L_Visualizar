...
```{r apiladasadmitidosexo-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Barras apiladas para la serie temporal del sexo de admitidos', warning=FALSE, message=FALSE, out.width='80%'}
admitidos <- UnalData::Aspirantes %>% 
  filter(ADMITIDO == 'Sí', YEAR != '2021') %>% 
  group_by(YEAR, SEXO) %>% 
  summarise(CANTIDAD = n()) 
ggplot(admitidos, aes(x = factor(YEAR), y = CANTIDAD, fill = SEXO)) +
  geom_bar(stat = 'identity', position = 'fill') +
  labs(x = 'Año',
       y = 'Estudiantes admitidos',
       title = 'Distribución de estudiantes admitidos por sexo') +
  scale_fill_manual(values=c("#f15a24", "#8cc63f"), name = "") +
  theme_bw() +
  theme(legend.position = 'bottom',
        plot.title = element_text(hjust = 0.5))
...
```{r apiladasadmitidosexonivel-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Distribución de estudiantes admitidos por sexo y nivel de formación', warning=FALSE, message=FALSE, out.width='80%'}
admitidos <- UnalData::Aspirantes %>% 
  filter(ADMITIDO == 'Sí') %>% 
  group_by(NIVEL, SEXO) %>% 
  summarise(CANTIDAD = n()) 
admitidos$NIVEL <- factor(admitidos$NIVEL, levels = c('Especialidades médicas',
                                                      'Especialización',
                                                      'Maestría',
                                                      'Doctorado',
                                                      'Pregrado'))
ggplot(admitidos, aes(x = NIVEL, y = CANTIDAD, fill = SEXO)) +
  geom_bar(stat = 'identity', position = 'fill') +
  labs(x = '',
       y = 'Estudiantes admitidos',
       title = 'Distribución de estudiantes admitidos por sexo y nivel de formación') +
  scale_fill_manual(values=c("#f15a24", "#8cc63f"), name = "") +
  coord_flip() +
  theme_bw() +
  theme(legend.position = 'bottom',
...
```{r agrupadasnivel-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Distribución de estudiantes admitidos por nivel de formación para los últimos tres años', warning=FALSE, message=FALSE, out.width='80%'}
graduados <- UnalData::Graduados %>% 
  filter(YEAR %in% c('2018', '2019', '2020')) %>% 
  group_by(YEAR, NIVEL) %>% 
  summarise(CANTIDAD = n()) %>% 
  transmute(YEAR, NIVEL, PORCENTAJE = round((CANTIDAD/sum(CANTIDAD))*100, 2))
ggplot(graduados, aes(x = factor(YEAR), y = PORCENTAJE, fill = NIVEL)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  scale_fill_manual(values = c("#6d6666", "#fbb03b", "#29abe2", "#c1272d", 
                               "#8cc63f"), name = '') +
  theme_bw() +
  ...
  labs(x = "", 
       ...
       title = "Distribución de estudiantes graduados por nivel de formación")
```
